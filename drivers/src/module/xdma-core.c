					 ...
					 struct xdma_desc **desc_last_p)
{
	...
	desc_virt = (struct xdma_desc *)pci_alloc_consistent(
				dev,
				number * sizeof(struct xdma_desc),
				desc_bus_p
			);
#else
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(5,17,15)
		pci_free_consistent(
			lro->pci_dev, sizeof(struct xdma_poll_wb),
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(5,17,15)
	engine->poll_mode_addr_virt = pci_alloc_consistent(
										lro->pci_dev,
										sizeof(struct xdma_poll_wb),
										&engine->poll_mode_bus
									);
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(5,17,15)
	engine->rx_result_buffer_virt = pci_alloc_consistent(
										lro->pci_dev,
										RX_RESULT_BUF_SIZE,
										&engine->rx_result_buffer_bus
									);
#else
	engine->rx_result_buffer_virt = dma_alloc_coherent(
										&lro->pci_dev->dev,
										RX_RESULT_BUF_SIZE,
										&engine->rx_result_buffer_bus,
										GFP_ATOMIC
...
static int __init xdma_init(void)
{
	...
	pr_info(DRV_NAME " v" DRV_MOD_VERSION "\n");

	dbg_init(DRV_NAME " init()\n");
	/* dbg_init(DRV_NAME " built " __DATE__ " " __TIME__ "\n"); */

#if LINUX_VERSION_CODE <= KERNEL_VERSION(6,3,9)
